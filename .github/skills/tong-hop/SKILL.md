...
After every completed pipeline run, perform these two actions automatically.

### Session Summary

```yaml
SESSION_SUMMARY:
  trigger: Pipeline completed (Step 5 done)
  
  check_existing:
    - Look for output/session-summary.md
    - If exists: append new session entry
    - If not exists: create file with header + first entry
    
  create_file:
    path: output/session-summary.md
    format: |
      # InsightEngine — Session Summary
      
      <!-- Auto-generated by tong-hop skill -->
  ...
  append_entry:
    # When file exists, append new session block at the end
    format: Same as create_file entry block
    
  COPILOT_MUST:
    - Always create/append session summary after pipeline completes
...
After creating the summary, suggest how the user can open the output file(s):
  ...
  per_format:
    docx:
      message: |
        ...
        → Cài extension **"Word Document Viewer"** (vscode:extension/cweijan.vscode-office)
           hoặc tải file về máy và mở bằng Microsoft Word / LibreOffice
        → Lệnh tải: right-click file trong Explorer → "Download..."
    ...
    pdf:
      message: |
        ...
        → Hoặc cài **"PDF Preview"** (vscode:extension/tomoki1207.pdf)
    ...
    html:
      message: |
        ...
        → Mở bằng **Simple Browser** (built-in):
           ...
           Hoặc click phải vào file → "Open with..." → Simple Browser
        → Hoặc cài **"Live Preview"** (vscode:extension/ms-vscode.live-server)
           Sau khi cài: click phải vào file → "Show Preview"
        → Lệnh mở trực tiếp (terminal):
           code --open-url "vscode-webview://..."  (dùng nếu biết URL nội bộ)
    ...
    multiple_files:
      # When pipeline produced multiple output formats:
      header: "📂 **Các file đầu ra — cách mở:**"
      list_each: Apply per_format suggestion for each file

COPILOT_MUST:
  - ALWAYS show view suggestions after pipeline completes
  - For HTML output: ALWAYS mention Simple Browser option (no extension needed)
  - For docx/xlsx/pptx: ALWAYS mention download option as fallback
  - Use Vietnamese for suggestion messages
  - Show extension marketplace ID so user can find it quickly
```
